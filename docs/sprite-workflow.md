...
- `parts`: part ID、grid cell、part内anchor、canvas上のposition、z-order、frameごとのinteger offset

`anchor`、`position`、`offsets`はcanvas-spaceのpixel整数とする。親子transformやsubpixel補間は行わない。同じ入力PNGとmanifestから同じdecode後RGBAとJSONを生成する。

標準idleではheadを`[0, 0, 1, 1, 2, 2, 1, 0]`pixel下げ、bodyとarmを`[0, 0, 1, 1, 1, 1, 0, 0]`pixel下げる。legは接地を守るため動かさず、bodyとの重なりで沈みを表現する。
...
- frameは横一列へpackする。
- transformはinteger translationだけとし、回転、拡縮、mesh変形、IKは行わない。
- editorはmask描画、partの回転・拡縮、IK、自動rig推定を行わない。
