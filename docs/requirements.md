...
Pixel Forgeは、写真を機械的かつ再現可能なピクセルアートへ変換するローカルツールである。macOS SwiftUIアプリでは画像の読み込み、調整、比較、履歴管理、書き出しを一つの制作フローとして提供し、同じRustエンジンを個人利用のCLIからも利用する。

対象は、写真からゲーム素材を繰り返し作る個人制作者である。アカウントやクラウドを前提にせず、入力画像と生成画像はMacの中だけで扱う。
...
- 表示設定、購入、サポート、アプリ情報はmacOS標準の設定ウインドウへまとめる。
- ホームでは画像を選ぶ、またはdrag and dropで読み込める。「アップロード」ではなく「画像を選ぶ」「読み込む」と表現する。
- ホームには生成画像を新しい順で2列に並べる。ウインドウ幅が不足する場合だけ1列へ切り替える。
...
- 同名ファイルが存在する場合はmacOSのsave panel上でユーザーに確認し、黙って上書きしない。

## 買い切りPro

- macOSアプリは無料で配布し、StoreKit 2のNon-Consumable商品`Pixel Forge Pro`を一度購入するとPro機能を解放する。
...
- Pro権利がない状態ではthemeをmacOSの外観への自動追従へ戻す。

## 設定とサポート

設定ウインドウは次のセクションを持つ。

- 表示: macOSに合わせる、黒基調、白基調
